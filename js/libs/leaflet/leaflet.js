...
(function(a){
    a.L={
        VERSION:"0.3",
        ROOT_URL:a.L_ROOT_URL||function(){
            var a=document.getElementsByTagName("script"),
                c=/\/?leaflet\-?([\w\-]*)\.js\??/,
                d,e,f,g;
            d=0;
            for(e=a.length;d<e;d++)if(f=a[d].src,g=f.match(c)){
                if(g[1]==="include")return"../../dist/";
                return f.replace(c,"")+"/"
            }
            return""
        }(),
...
L.Util={
    ...
    template:function(a,b){
        return a.replace(/\{ *([^} ]+) *\}/g,function(a,d){
            var e=b[d];
            if(!b.hasOwnProperty(d))throw Error("No value provided for variable "+a);
            return e
        })
    }
};
...
L.Mixin.Events={
    addEventListener:function(a,b,c){
        ...
        d[a]=d[a]||[];
        d[a].push({action:b,context:c||this});
        return this
    },
    ...
    fireEvent:function(a,b){
        if(this.hasEventListeners(a)){
            for(var c=L.Util.extend({type:a,target:this},b),d=this._leaflet_events[a].slice(),e=0,f=d.length;e<f;e++)d[e].action.call(d[e].context||this,c);
            return this
        }
    }
};
...
(function(){
    ...
    L.Browser={
        ...
        touch:function(){
            var a=!1;
            if("ontouchstart"in document.documentElement)return!0;
            var b=document.createElement("div");
            if(!b.setAttribute||!b.removeAttribute)return!1;
            b.setAttribute("ontouchstart","return;");
            typeof b.ontouchstart=="function"&&(a=!0);
            b.removeAttribute("ontouchstart");
            return a
        }()
    }
})();
...
a instanceof L.Bounds?(b=a.min,a=a.max):b=a;return b.x>=this.min.x&&a.x<=this.max.x&&b.y>=this.min.y&&a.y<=this.max.y}});L.Transformation=L.Class.extend({initialize:function(a,b,c,d){this._a=a;this._b=b;this._c=c;this._d=d},
transform:function(a,b){return this._transform(a.clone(),b)},
_transform:function(a,b){b=b||1;a.x=b*(this._a*a.x+this._b);a.y=b*(this._c*a.y+this._d);return a},
untransform:function(a,b){b=b||1;return new L.Point((a.x/b-this._b)/this._a,(a.y/b-this._d)/this._c)}});
...
L.LineUtil={
    ...
    simplifyDP:function(a,b){
        var c=0,
            ...
            h;
        if(f<3)return a;
        for(g=0;g<f-1;g++)h=this._sqPointToSegmentDist(a[g],a[0],a[f-1]),h>c&&(d=g,c=h);
        return c>=e?(c=a.slice(0,d),d=a.slice(d),c=this.simplifyDP(c,b),d=this.simplifyDP(d,b),c.concat(d)):[a[0],a[f-1]]
    },
...
L.DomEvent={
    addListener:function(a,b,c,d){
        ...
        if(!a[g]){
            if(L.Browser.touch&&b=="dblclick"&&this.addDoubleTapListener)this.addDoubleTapListener(a,e,f);
                ...
                var h=e,
                    e=function(b){
                        if(L.DomEvent._checkMouse(a,b))return h(b)
                    };
    ...
    _checkMouse:function(a,b){
        var c=b.relatedTarget;if(!c)return!0;
        try{
    ...
    stopPropagation:function(a){a.stopPropagation?a.stopPropagation():a.cancelBubble=!0},disableClickPropagation:function(a){L.DomEvent.addListener(a,
    "mousedown",L.DomEvent.stopPropagation);L.DomEvent.addListener(a,"click",L.DomEvent.stopPropagation);L.DomEvent.addListener(a,"dblclick",L.DomEvent.stopPropagation)},preventDefault:function(a){a.preventDefault?a.preventDefault():a.returnValue=!1},
...
L.DomUtil={
    get:function(a){
        return typeof a=="string"?document.getElementById(a):a},getStyle:function(a,b){var c=a.style[b];
    ...
    getViewportOffset:function(a){
        var b = 0,
            ...
            d = a,
            e = document.body;
        do if(b += d.offsetTop || 0, c+=d.offsetLeft || 0, d.offsetParent == e&&L.DomUtil.getStyle(d, "position") == "absolute") break;
    ...
    a.className=a.className.replace(/(\S+)\s*/g,function(a,d){if(d==b)return"";return a}).replace(/^\s+/,"")
    },
...
L.Util.extend(L.DomUtil,{
    TRANSITION:L.DomUtil.
    testProp(["transition","webkitTransition","OTransition","MozTransition","msTransition"]),
...
L.Draggable=L.Class.extend({
    ...
    _onDown:function(a){
        if(!(!L.Browser.touch&&a.shiftKey||a.which!=1&&a.button!=1&&!a.touches)&&!(a.touches&&a.touches.length>1)){
            var b=a.touches&&a.touches.length==1?a.touches[0]:a,
                c=b.target;
            L.DomEvent.preventDefault(a);
            L.Browser.touch&&c.tagName.toLowerCase()==="a"&&(c.className+=" leaflet-active");
            this._moved=!1;
            if(!this._moving)L.Browser.touch||(L.DomUtil.disableTextSelection(),this._setMovingCursor()),this._startPos=this._newPos=L.DomUtil.getPosition(this._element),
            this._startPoint=new L.Point(b.clientX,b.clientY),L.DomEvent.addListener(document,L.Draggable.MOVE,this._onMove,this),L.DomEvent.addListener(document,L.Draggable.END,this._onUp,this)
        }
    },
    ...
    _onMove:function(a){
        if(!(a.touches&&a.touches.length>1)){
            ...
            if(!this._moved)this.fire("dragstart"),this._moved=!0;this._moving=!0;this._newPos=this._startPos.add(new L.Point(a.clientX,a.clientY)).subtract(this._startPoint);
            L.Util.requestAnimFrame(this._updatePosition,this,!0,this._dragStartTarget)
        }
    },
    ...
    _onUp:function(a){
        if(a.changedTouches){
            ...
            if(b.tagName.toLowerCase()==="a")b.className=b.className.replace(" leaflet-active","");
        ...
        this._moved&&this.fire("dragend");this._moving=!1
    },
    ...
    L.Transition=L.Class.extend({
        includes:L.Mixin.Events,statics:{
...
L.LatLng=function(a,b,c){
	...
	c!==!0&&(d=Math.max(Math.min(d,90),-90),
			e=(e+180)%360+(e<-180?180:-180));
	this.lat=d;
	this.lng=e
};
...
this.fire("move");clearTimeout(this._sizeTimer);this._sizeTimer=setTimeout(L.Util.bind(function(){this.fire("moveend")},this),200);return this},getCenter:function(a){var b=this.getSize().divideBy(2);return this.unproject(this._getTopLeftPoint().add(b),this._zoom,a)},getZoom:function(){return this._zoom
},
...
getMaxZoom:function(){
    ...
    return Math.min(a,this._layersMaxZoom||Infinity)
},
...
getBoundsZoom:function(a,b){
    var c=this.getSize(),
        ...
        h,j;
    h=!0;b&&d--;
    do d++,h=this.project(f,d),j=this.project(g,d),h=new L.Point(h.x-j.x,j.y-h.y),h=b?h.x<c.x||h.y<c.y:h.x<=c.x&&h.y<=c.y;while(h&&d<=e);if(h)return null;
    return b?d:d-1
},
...
this._update,this):this._map.off("move",this._limitedUpdate,this)},getAttribution:function(){return this.options.attribution},setOpacity:function(a){this.options.opacity=a;this._setOpacity(a);if(L.Browser.webkit)for(i in this._tiles)this._tiles[i].style.webkitTransform+=" translate(0,0)"},_setOpacity:function(a){a<1&&L.DomUtil.setOpacity(this._container,a)},_initContainer:function(){var a=this._map.getPanes().tilePane,b=a.firstChild;if(!this._container||a.empty)this._container=L.DomUtil.create("div",
...
_update:function(){
    var a=this._map.getPixelBounds(),
        ...
        c=new L.Bounds(c,a);
        if(c.max.x>140000){
            c.max.x = c.max.x-155173;
            c.min.x = c.min.x-155173;
        }
...
_addTilesFromCenterOut:function(a){
    for(var b=[], c=a.getCenter(), d=a.min.y; 
            d<=a.max.y; 
            d++)for(var e=a.min.x; 
                        e<=a.max.x; 
                        e++) e+":"+d in this._tiles||b.push(new L.Point(e,d));
    b.sort( function(a,b){ return a.distanceTo(c) - b.distanceTo(c) });
    a = document.createDocumentFragment();
    this._tilesToLoad = b.length;d=0;
    for(e=this._tilesToLoad;d<e;d++){
        this._addTile(b[d],a);this._container.appendChild(a);
    }
},
...
_removeOtherTiles:function(a){
    ...
    for (d in this._tiles){
        if(this._tiles.hasOwnProperty(d)&&(b=d.split(":"),c=parseInt(b[0],10),b=parseInt(b[1],10),c<a.min.x||c>a.max.x||b<a.min.y||b>a.max.y)){
            c=this._tiles[d],this.fire("tileunload",{tile:c,url:c.src}),c.parentNode==this._container&&this._container.removeChild(c),delete this._tiles[d];
        }
    }
},
...
_addTile:function(a,b){
    ...
    this._loadTile(g,a,d);b.appendChild(g)
},
...
getTileUrl:function(a,b){
    return L.Util.template(
        this._url,L.Util.extend({
                s:this.options.subdomains[(a.x+a.y)%this.options.subdomains.length],
                z:b+this.options.zoomOffset,x:a.x,y:a.y
                },
            this._urlParams
        )
    )
...
_createTileProto:function(){
    ...
    this._tileImg.galleryimg="no";var a=this.options.tileSize;
...
_tileOnError:function(){var a=this._layer;a.fire("tileerror",{tile:this,url:this.src});
if(a=a.options.errorTileUrl)this.src=a}});
L.TileLayer.WMS=L.TileLayer.extend({defaultWmsParams:{service:"WMS",request:"GetMap",version:"1.1.1",layers:"",styles:"",format:"image/jpeg",transparent:!1},initialize:function(a,b){this._url=a;this.wmsParams=L.Util.extend({},this.defaultWmsParams);
this.wmsParams.width=this.wmsParams.height=this.options.tileSize;
for(var c in b)this.options.hasOwnProperty(c)||(this.wmsParams[c]=b[c]);L.Util.setOptions(this,b)},onAdd:function(a){this.wmsParams[parseFloat(this.wmsParams.version)>=1.3?"crs":"srs"]=a.options.crs.code;
...
this._containerLeft=a.x-Math.round(this._containerWidth/2)+this.options.offset.x;this._container.style.bottom=this._containerBottom+"px";this._container.style.left=this._containerLeft+"px"},_adjustPan:function(){if(this.options.autoPan){var a=this._container.offsetHeight,b=this._map.layerPointToContainerPoint(new L.Point(this._containerLeft,-a-this._containerBottom)),c=new L.Point(0,0),d=this.options.autoPanPadding,e=this._map.getSize();if(b.x<0)c.x=b.x-d.x;if(b.x+this._containerWidth>e.x)c.x=b.x+
this._containerWidth-e.x+d.x;if(b.y<0)c.y=b.y-d.y;if(b.y+a>e.y)c.y=b.y+a-e.y+d.y;(c.x||c.y)&&this._map.panBy(c)}},_onCloseButtonClick:function(a){this._close();L.DomEvent.stop(a)}});L.Icon=L.Class.extend({iconUrl:L.ROOT_URL+"images/marker.png",shadowUrl:L.ROOT_URL+"images/marker-shadow.png",iconSize:new L.Point(25,41),shadowSize:new L.Point(41,41),iconAnchor:new L.Point(13,41),popupAnchor:new L.Point(0,-33),initialize:function(a){if(a)this.iconUrl=a},createIcon:function(){return this._createIcon("icon")},createShadow:function(){return this._createIcon("shadow")},_createIcon:function(a){var b=this[a+"Size"],c=this[a+"Url"],d=this._createImg(c);if(!c)return null;d.className="leaflet-marker-"+
...
d.setAttribute("viewBox",[b.x,b.y,a,c].join(" "));L.Browser.webkit&&e.appendChild(d)}});L.Browser.vml=function(){var a=document.createElement("div");a.innerHTML='<v:shape adj="1"/>';a=a.firstChild;a.style.behavior="url(#default#VML)";return a&&typeof a.adj=="object"}();
...
this._parts.length;a<c;a++){b=0;for(d=this._parts[a].length;b<d;b++)e=this._parts[a][b],f=(b===0?"move":"line")+"To",this._ctx[f](e.x,e.y);this instanceof L.Polygon&&this._ctx.closePath()}},_checkIfEmpty:function(){return!this._parts.length},_updatePath:function(){if(!this._checkIfEmpty()){this._drawPath();this._ctx.save();this._updateStyle();var a=this.options.opacity,b=this.options.fillOpacity;if(this.options.fill){if(b<1)this._ctx.globalAlpha=b;this._ctx.fill()}if(this.options.stroke){if(a<1)this._ctx.globalAlpha=
...
L.Handler.MapDrag=L.Handler.extend({
    ...
    _onViewReset:function(){
        ...
        this._initialWorldOffset=this._map.latLngToLayerPoint(new L.LatLng(0,0)).subtract(a)
    },
    ...
    _onPreDrag:function(){
        var a=this._map,
            ...
            c=Math.round(b/2),
            a=this._initialWorldOffset.x,
            d=this._draggable._newPos.x,
            e=(d-c+a)%b+c-a,
            b=(d+c+a)%b-c-a;
        this._draggable._newPos.x=Math.abs(e+a)<Math.abs(b+a)?e:b
    ...
    _onDragEnd:function(){
        ...
        a.fire("moveend");
        a.fire("dragend");
        this._map.options.maxBounds&&L.Util.requestAnimFrame(this._panInsideMaxBounds,a,!0,a._container)
    },
...
L.Handler.ScrollWheelZoom=L.Handler.extend({
    ...
    _performZoom:function(){
        var a=Math.round(this._delta),
            ...
            a=this._map._limitZoom(b+a)-b;
        this._delta=0;
        a && this._map.setView(this._getCenterForScrollWheelZoom(this._lastMousePos,a),b+a)
    },
